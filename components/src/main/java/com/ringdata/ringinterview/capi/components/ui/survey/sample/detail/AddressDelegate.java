...
public class AddressDelegate extends LatteDelegate {
    ...
    public Object setLayout() {
        return R.layout.delegate_sample_detail_address;
    }
    ...
    private void initView() {
        recyclerView = (RecyclerView) mRootView.findViewById(R.id.rv_sample_address);
    ...
    private void initData() {
        ...
        LinkedList<MultiItemEntity> addressList = SampleAddressDao.queryBySampleGuid(currentSampleId, SPUtils.getInstance().getInt(SPKey.USERID));
        for (int i = 0; i < addressList.size(); i++) {
            int position = i + 1;
            PropertyListBean title = new PropertyListBean(SampleDetailItemType.LIST_TITLE_BEAN, SampleDetailItemType.ADDRESS, addressList.get(i), "地址" + position, checkIsCanEdit(addressList.get(i)), i);
            list.add(title);
    ...
    private boolean checkIsCanEdit(MultiItemEntity item) {
        ...
        return sampleAddress.getCreateUser().equals(currentUserId);
    }
    ...
    void onClickAdd() {
        start(EditAddressDelegate.newInstance(null));
    }

    @Subscribe(threadMode = ThreadMode.MAIN)
    public void onMessageEvent(MessageEvent event) {
        if (event.getTag() == Msg.REFRESH_ADDRESS) {
            initData();
        }
    }
