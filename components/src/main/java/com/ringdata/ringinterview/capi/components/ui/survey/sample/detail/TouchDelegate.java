...
public class TouchDelegate extends LatteDelegate {
    ...
    public Object setLayout() {
        return R.layout.delegate_sample_detail_touch;
    }
    ...
    private void initView() {
        recyclerView = (RecyclerView) mRootView.findViewById(R.id.rv_sample_record);
    ...
    private void initData() {
        ...
        LinkedList<MultiItemEntity> touchList = SampleTouchDao.queryBySampleGuid(currentSampleId, SPUtils.getInstance().getInt(SPKey.USERID));
        for (int i = 0; i < touchList.size(); i++) {
            int position = i + 1;
            PropertyListBean title = new PropertyListBean(SampleDetailItemType.LIST_TITLE_BEAN, SampleDetailItemType.TOUCH, touchList.get(i), "接触记录" + position, checkIsCanEdit(touchList.get(i)), i);
            list.add(title);
            list.add(touchList.get(i));
        }
    ...
    private boolean checkIsCanEdit(MultiItemEntity item) {
        ...
        return sampleTouch.getCreateUser().equals(currentUserId);
    }
    ...
    public void onMessageEvent(MessageEvent event) {
        if (event.getTag() == Msg.REFRESH_TOUCH) {
            initData();
        }
    }
