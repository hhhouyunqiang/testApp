...
public class BottomMenuDialog extends Dialog{
    private IGlobalCallback onClick = null;
    ...
    public static class Builder {
        ...
        public Builder addMenus(ArrayList<BottomMenuBean> menuBeans,final IGlobalCallback onClick ) {
         for(int i = 0; i < menuBeans.size(); i++){
             final int finalI = menuBeans.get(i).getTag();
             BottomMenu bm = new BottomMenu(menuBeans.get(i).getText(), new View.OnClickListener() {
                 @Override
                 public void onClick(View v) {
                     onClick.executeCallback(finalI);
                 }
              });
             p.menuList.add(bm);
             }
        ...
        public BottomMenuDialog create() {
            ...
            View view = LayoutInflater.from(p.context).inflate(R.layout.base_dialog_bottom_menu, null);
            ...
            if (hasTitle) {
                ...
                tTitle.setBackgroundResource(R.drawable.base_dialog_bottom_menu_top);
            ...
            for (int i = 0; i < p.menuList.size(); i++) {
                ...
                int backgroundResId = R.drawable.base_dialog_bottom_menu_center;
                if (p.menuList.size() > 1) {
                    if (i == 0) {
                        if (hasTitle) {
                            backgroundResId = R.drawable.base_dialog_bottom_menu_center;
                        } else {
                            backgroundResId = R.drawable.base_dialog_bottom_menu_top;
                        }
                    } else if (i == p.menuList.size() - 1) {
                        backgroundResId = R.drawable.base_dialog_bottom_menu_bottom;
                    }
                } else if (p.menuList.size() == 1) {
                    backgroundResId = R.drawable.base_dialog_bottom_menu_singleton;
                }
