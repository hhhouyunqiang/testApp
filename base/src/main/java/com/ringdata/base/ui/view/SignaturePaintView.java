...
public class SignaturePaintView extends View {
	...
	// 这里的构造一定是两个参数。
	public SignaturePaintView(final Context ctxt, AttributeSet attrs) {
		super(ctxt, attrs);
		DisplayMetrics dm = new DisplayMetrics();
		if (ctxt instanceof Activity) {
			Activity order = (Activity) ctxt;
			order.getWindowManager().getDefaultDisplay().getMetrics(dm);
		}
		// getWindowManager().getDefaultDisplay().getMetrics(dm);
		// PaintView(ctxt, dm.widthPixels, dm.heightPixels);
	...
	public SignaturePaintView(Context context, int w, int h) {
		super(context);
		screenWidth = w;
		screenHeight = h;
